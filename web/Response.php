 ...
 *
 * Response is configured as an application component in [[\izi\web\Application]] by default.
 * You can access that instance via `Phal::$app->response`.
 *
 ...
 * ```php
 * 'response' => [
 *     'format' => izi\web\Response::FORMAT_JSON,
 *     'charset' => 'UTF-8',
 *     // ...
 * ]
 * ```
...
class Response extends \izi\base\Response
{
     ...
     * This property is mainly set by [[\izi\filters\ContentNegotiator]].
     */
    ...
    protected function sendCookies()
    {
        ...
        foreach ($this->getCookies() as $cookie) {
            ...
            if ($cookie->expire != 1 && isset($validationKey)) {
                $value = Phal::$app->getSecurity()->hashData(serialize([$cookie->name, $value]), $validationKey);
            }
     ...
     * {
     *     $storagePath = Phal::getAlias('@app/files');
     *
     *     // check filename for allowed chars (do not allow ../ to avoid security issue: downloading arbitrary files)
     *     if (!preg_match('/^[a-z0-9]+\.[a-z0-9]+$/i', $filename) || !is_file("$storagePath/$filename")) {
     *         throw new \izi\web\NotFoundHttpException('The file does not exists.');
     *     }
     *     return Phal::$app->response->sendFile("$storagePath/$filename", $filename);
     * }
     * ```
     *
    ...
    {
        $rangeHeader = Phal::$app->getRequest()->getHeaders()->get('Range', '-');
        if ($rangeHeader === '-') {
            return [0, $fileSize - 1];
        }
     ...
     *
     * ```php
     * Phal::$app->response->xSendFile('/home/user/Pictures/picture1.jpg');
     * ```
     *
     ...
     *
     * ```php
     * return Phal::$app->getResponse()->redirect($url);
     * ```
     *
     ...
     *
     * ```php
     * Phal::$app->getResponse()->redirect($url)->send();
     * return;
     * ```
     *
    ...
    public function redirect($url, $statusCode = 302, $checkAjax = true)
    {
        ...
        if (strncmp($url, '/', 1) === 0 && strncmp($url, '//', 2) !== 0) {
            $url = Phal::$app->getRequest()->getHostInfo() . $url;
        }

        if ($checkAjax) {
            if (Phal::$app->getRequest()->getIsAjax()) {
                if (Phal::$app->getRequest()->getHeaders()->get('X-Ie-Redirect-Compatibility') !== null && $statusCode === 302) {
                    // Ajax 302 redirect in IE does not work. Change status code to 200. See https://github.com/yiisoft/yii2/issues/9670
                    $statusCode = 200;
                }
                if (Phal::$app->getRequest()->getIsPjax()) {
     ...
     *
     * ```php
     * return Phal::$app->getResponse()->refresh();
     * ```
     *
    ...
    {
        return $this->redirect(Phal::$app->getRequest()->getUrl() . $anchor);
    }
    ...
    protected function defaultFormatters()
    {
        return [
            self::FORMAT_HTML => [
                'class' => 'izi\web\HtmlResponseFormatter',
            ],
            self::FORMAT_XML => [
                'class' => 'izi\web\XmlResponseFormatter',
            ],
            self::FORMAT_JSON => [
                'class' => 'izi\web\JsonResponseFormatter',
            ],
            self::FORMAT_JSONP => [
                'class' => 'izi\web\JsonResponseFormatter',
                'useJsonp' => true,
    ...
    protected function prepare()
    {
        ...
        if (isset($this->formatters[$this->format])) {
            ...
            if (!is_object($formatter)) {
                $this->formatters[$this->format] = $formatter = Phal::createObject($formatter);
            }

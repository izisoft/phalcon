...
use izi\base\Exception;
use izi\base\UserException;
 ...
 *
 * ErrorHandler is configured as an application component in [[\izi\base\Application]] by default.
 * You can access that instance via `Phal::$app->errorHandler`.
 *
 ...
 * @since 2.0
 */
class ErrorHandler extends \izi\base\ErrorHandler
{
    /**
     ...
     * using `Phal::$app->errorHandler->exception`. This property defaults to null, meaning ErrorHandler
    ...
    {
        if (Phal::$app->has('response')) {
            $response = Phal::$app->getResponse();
        ...
        if ($useErrorView && $this->errorAction !== null) {
            $result = Phal::$app->runAction($this->errorAction);
    ...
    protected function convertExceptionToArray($exception)
    {
        if (!YII_DEBUG && !$exception instanceof UserException && !$exception instanceof HttpException) {
            $exception = new HttpException(500, Phal::t('yii', 'An internal server error occurred.'));
        }
        ...
        if (YII_DEBUG) {
            ...
            if (!$exception instanceof UserException) {
                ...
                if ($exception instanceof \izi\db\Exception) {
    ...
    public function renderFile($_file_, $_params_)
    {
        ...
        if ($this->exception instanceof ErrorException || !Phal::$app->has('view')) {
            ob_start();
            ob_implicit_flush(false);
            extract($_params_, EXTR_OVERWRITE);
            require Phal::getAlias($_file_);

            return ob_get_clean();
        }

        return Phal::$app->getView()->renderFile($_file_, $_params_, $this);
    }
    ...
    {
        return '<a href="http://github.com/yiisoft/yii2/" target="_blank">' . $this->htmlEncode(Phal::getVersion()) . '</a>';
    }
    ...
    {
        if ($exception instanceof \izi\base\Exception || $exception instanceof \izi\base\InvalidCallException || $exception instanceof \izi\base\InvalidParamException || $exception instanceof \izi\base\UnknownMethodException) {
    ...
    {
        return YII_ENV_TEST || Phal::$app->request->getIsAjax();
    }
}
